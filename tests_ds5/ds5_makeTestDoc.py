...
for c in [a1.minimum, a1.maximum]:
	for d1 in a2.values:
		for d2 in a3.values:
			missingSource = ""
			if d1 == 1 and d2 == 1 and c == a1.maximum:
				# make a source descriptor with a missing source for this discrete location
				missingSource = "_missingSource_"
			s1 = SourceDescriptor()
			s1.path = os.path.join("sources", f"geometrySource_c_{c}_d1_{d1}_d2_{d2}{missingSource}.ufo")
